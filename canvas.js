...
const drawTree = (ctx) => {
    const now = new Date();
    const theta = -(now.getMilliseconds() + now.getSeconds() * 1000 + now.getMinutes() * 60 * 1000 + now.getHours() * 60 * 60 * 1000) * 2 * Math.PI * speed / 1000;
    // console.log(theta);
    ...
    const line2 = matMult(rotationMatrix(theta / 60), [0, startLength * document.getElementById("sslider").value / 100]);
    const line3 = matMult(rotationMatrix(theta / 60 / 60), [0, startLength * document.getElementById("mslider").value / 100]);
    const line4 = matMult(rotationMatrix(theta / 60 / 60 / 12), [0, startLength * document.getElementById("hslider").value / 100]);
    ...
    const transMatrix4 = rotationMatrix(Math.atan2(line0[1], line0[0]) - Math.atan2(line4[1], line4[0])).map(vector => vector.map(x => x * length(line4) / length(line0)));
    ...
    while (lines.length > 0) {
        ...
        if (document.getElementById("sslider").value > 0) lines.push([line[1], add(line[1], matMult(transMatrix2, sub(line[0], line[1]))), line[2] + 1]);
        if (document.getElementById("mslider").value > 0) lines.push([line[1], add(line[1], matMult(transMatrix3, sub(line[0], line[1]))), line[2] + 1]);
        if (document.getElementById("hslider").value > 0) lines.push([line[1], add(line[1], matMult(transMatrix4, sub(line[0], line[1]))), line[2] + 1]);
    }
