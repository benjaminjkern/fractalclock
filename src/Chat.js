    ...
    */
    return (
        <>
            <div
                id="chat"
                style="position: absolute; height: 100vh; width: 20%; right: 0%; background-color: #00000033; opacity: 0"
            >
                <div
                    id="top"
                    style="position: absolute; width: 100%; height: max(40px, 2em); background-color: #00000033; text-align: center; line-height: max(40px, 2em);"
                >
                    Chat
                    <div
                        id="closeChat"
                        style="position: absolute; width: max(40px, 2em); height: max(40px, 2em); right: 0; top: 0; background-color:#ff000077; text-align:center; line-height: max(40px, 2em); user-select: none;"
                    >
                        X
                    </div>
                </div>
                <div
                    id="chatBox"
                    style="overflow-y:scroll; overflow-x: hidden; height: calc(100% - max(110px, 5.5em)); width: calc(100% - max(20px, 1em)); position: absolute; top: max(50px, 2.5em); left: max(10px, 0.5em);"
                >
                    <div style="color:darkgrey; line-height: calc(100vh - max(120px, 6em)); text-align: center;">
                        There are no chats!
                    </div>
                </div>
                <div style="position: absolute; width: calc(100% - max(20px, 1em)); height: max(40px, 2em); bottom: max(10px, 0.5em); left: max(10px,0.5em); background-color: #00000033; border-radius: max(20px, 1em);">
                    <input
                        id="text"
                        style="border: none; height: max(30px, 1.5em); font-size: 1em; outline: none; background-color:#00000000; margin-top: max(5px, 0.25em); margin-left: max(10px, 0.5em); width: calc(100% - max(50px, 2.5em)); color: white;"
                        placeholder="Type a message"
                    ></input>
                    <div
                        id="send"
                        style="position: absolute; width: max(30px, 1.5em); height: max(30px, 1.5em); right: max(5px, 0.25em); top: max(5px, 0.25em); border-radius: max(15px, 0.75em); background-color: #fff; color: #000; line-height: max(30px, 1.5em); text-align: center; user-select: none;"
                    >
                        &gt;
                    </div>
                </div>
            ...
            <div
                id="openChat"
                style="position: absolute; width: max(20px, 1em); height: max(40px, 2em); right: 0; top: calc(50% - max(20px, 1em)); background-color:#00000033; border-radius: max(10px, 0.5em) 0 0 max(10px, 0.5em); text-align:center; 
    line-height: max(40px, 2em); user-select: none;"
            >
                &lt;
            </div>
        </>
...
//         if (!window.chatOpen || window.phoneScreen)
//             chatOpenButton.style.opacity = "1";
//         setTimeout(() => {
//             if (
//                 new Date().getTime() - timeSinceMove > wait &&
//                 chatOpenButton.style.backgroundColor === "rgba(0, 0, 0, 0.2)" &&
//                 !window.chatOpen &&
//                 !window.phoneScreen
//             ) {
//                 chatOpenButton.style.opacity = "0";
